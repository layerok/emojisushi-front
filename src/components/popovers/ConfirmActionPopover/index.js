...
export const ConfirmActionPopover = (
    ...
    return <NestedPopover render={({close}) => (<S.Wrapper>
        ...
        <FlexBox style={{marginTop: "20px"}} justifyContent={"space-between"}>
            <ButtonOutline onClick={() => onConfirm({close})} width={"162px"}>{t('confirmAction.yes')}</ButtonOutline>
            <ButtonFilled onClick={() => onCancel({close})} width={"162px"}>{t('confirmAction.no')}</ButtonFilled>
        </FlexBox>
